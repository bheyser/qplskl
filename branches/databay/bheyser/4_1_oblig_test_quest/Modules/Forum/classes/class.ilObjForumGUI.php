...
class ilObjForumGUI extends ilObjectGUI
{
	...
	*/
	...
	public function updateObject()
	{
		...
		$this->objProperties->setSubjectSetting($_POST['subject_setting']);
	...
	//_force_new_post_titles
		...
		$frm_subject->setValue($this->objProperties->getSubjectSetting());
...
//			}
	...
	private function initNotificationSettingsForm()
	{
		if(null === $this->notificationSettingsForm)
		{
			...
			$chb_1 = new ilCheckboxInputGUI($this->lng->txt('admin_force_noti'), 'adm_force');
			$chb_1->setValue(1);

			$chb_2 = new ilCheckboxInputGUI($this->lng->txt('user_toggle_noti'), 'usr_toggle');
			$chb_2->setValue(1);

			$opt_0->addSubItem($chb_1);
	...
	//	if($parent_type == 'crs')
		...
		else
		{
			...
			foreach($member_ids as $user_id)
			{
				...
				$frm_noti->setAdminForce($this->objProperties->isAdminForceNoti());
