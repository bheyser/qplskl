...
*/
	...
	*/
	public function calculateResultsFromSolution($active_id, $pass = NULL)
	{
		...
		$affectedRows = $ilDB->manipulateF(
			$query, array("integer","integer", "integer", "integer", "float", "integer", 'integer'),
			array(
				$next_id,
				$active_id,
				$this->getId(),
	...
	*/
	...
	function _updateTestResultCache($active_id)
	{
		...
		$affectedRows = $ilDB->manipulateF( $query,
			array(
				'integer',
				'integer',
				'float',
	...
	function _updateTestPassResults($active_id, $pass)
	{
		...
		if ($result->numRows() > 0)
		{
			$query = 'SELECT count( * ) cnt, min( answered ) answ
				FROM tst_test_question
				INNER JOIN tst_active ON active_id = %s
				AND tst_test_question.test_fi = tst_active.test_fi
				LEFT JOIN tst_test_result ON tst_test_result.active_fi = %s AND tst_test_result.pass = %s AND tst_test_question.question_fi = tst_test_result.question_fi
				WHERE obligatory = 1';
